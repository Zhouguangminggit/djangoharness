...
`celery_app.celery` 导入 `__PROJECT_PACKAGE__.logging`，Python 随即执行
`__PROJECT_PACKAGE__/__init__.py`。该入口又反向导入尚未初始化完成的
`celery_app.celery.app`，因此触发循环导入。

## 变更

- 移除 `__PROJECT_PACKAGE__` 对 Celery 实例的反向导出，保持
  `celery_app -> __PROJECT_PACKAGE__.logging` 的单向依赖。

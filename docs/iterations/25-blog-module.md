...
- 新增 `apps/blog/` 应用，包含：
  ...
  - 后台：`apps/blog/admin.py` 注册模型；`base_framework/settings/admin.py` 在 Unfold 侧边栏新增“博客”菜单（发布文章、文章列表、分类、标签、作者）。
...
- 新增 CKEditor 5 配置：`base_framework/settings/base.py`。
- 挂载博客 URL 和 CKEditor 5 上传路由：`base_framework/urls.py`。
